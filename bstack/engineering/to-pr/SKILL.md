...
Use the repository's title convention. Otherwise use `type(scope): short imperative subject`, with a real changed area as the scope. Keep the briefing focused on the final change; include explored alternatives only to explain the chosen solution. Apply the sentence-level guidance from [technical-writing](../../upstream/pstack/skills/technical-writing/SKILL.md), then unslop. Its shorter PR-body target does not override this workflow's three-minute limit.
