...
For selected Matt Pocock sources, candidate discovery is limited by each manifest's `review_paths`. Check that a new file in a selected skill is reported, while an unrelated upstream skill is excluded. All three imports must compare unchanged against their exact pinned checkout, including the root license. This is source verification; the imported SDLC workflows, handoff, and support dependencies have not been activated or behaviorally verified across hosts.
