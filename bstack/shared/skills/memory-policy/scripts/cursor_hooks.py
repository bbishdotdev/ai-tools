...
def context_hook(home):
    script = home / 'Work/.agents/skills/memory-policy/scripts/cursor_context.py'
    return {
        'command': shlex.join(['python3', '-B', str(script), '--home', str(home)]),
...
def inspect_hooks(home):
    ...
    try:
        ...
        if not ready:
            raise SyncError('Cursor requires its current-snapshot beforeSubmitPrompt hook; run install.py --activate-sync')
