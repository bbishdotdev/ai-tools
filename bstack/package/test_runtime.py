...
class RuntimeTests(unittest.TestCase):
    ...
    def make_capsule(self, root):
        for directory in ("scripts", "runtime", "content/shared/skills/bstack-router", "content/shared/skills/unslop"):
        ...
        (root / "content/shared/skills/bstack-router/WORKFLOW.md").write_text("# Fixture router\n")
        (root / "content/shared/skills/unslop/WORKFLOW.md").write_text("# Customized unslop\n")
        (root / "SKILL.md").write_text("---\nname: poteto-mode\ndisable-model-invocation: true\n---\n\nRead the bundled router.\n")
    ...
    def refresh_manifest(self, root):
        ...
        (root / "manifest.json").write_text(json.dumps({"files": files, "entrypoints": {"router": "content/shared/skills/bstack-router/WORKFLOW.md"}}))

    def run_cli(self, *args, success=True, project=None, capsule=None, payload=None):
        result = subprocess.run([sys.executable, str((capsule or self.capsule) / "scripts/bstack.py"), *args,
                                 "--project", str(project or self.project)], input=json.dumps(payload) if payload else None,
    ...
    def test_initial_manual_setup_is_idempotent_and_doctor_checks(self):
        ...
        self.assertIn("unslop/WORKFLOW.md", instructions)
        self.assertNotIn("bstack-router/WORKFLOW.md", instructions)
        self.assertIn("Engineering routing is manual", instructions)
        self.assertIn("only when the user explicitly invokes Poteto Mode", instructions)
        self.assertFalse((self.project / ".codex/hooks.json").exists())
        self.assertFalse((self.project / ".agents/skills/poteto-mode").exists())
    ...
    def test_on_off_are_idempotent_and_unslop_survives(self):
        ...
        self.assertIn("bstack-router/WORKFLOW.md", (self.project / ".bstack/instructions.md").read_text())
        ...
        self.assertIn("unslop/WORKFLOW.md", (self.project / ".bstack/instructions.md").read_text())
        self.assertNotIn("bstack-router/WORKFLOW.md", (self.project / ".bstack/instructions.md").read_text())
        self.assertFalse((self.project / ".codex/hooks.json").exists())
    ...
    def test_setup_rebinds_updated_capsule_preserving_auto(self):
        ...
        self.assertIn(str(updated), (self.project / ".bstack/instructions.md").read_text())
        self.assertNotIn(str(self.capsule), (self.project / ".bstack/instructions.md").read_text())
        self.assertEqual(self.run_cli("doctor", capsule=updated)["bindings"], "passed")
    ...
    def test_off_hook_is_silent_and_on_hook_uses_installed_router(self):
        ...
        self.assertIn(str(self.capsule / "content/shared/skills/bstack-router/WORKFLOW.md"), output["hookSpecificOutput"]["additionalContext"])
        self.run_cli("auto", "off")
        self.assertEqual(self.run_cli("hook", "--host", "codex", payload=event), {})
    ...
    def test_project_state_is_isolated(self):
        ...
        self.assertNotIn("bstack-router/WORKFLOW.md", (other / ".bstack/instructions.md").read_text())

    def test_package_integrity_prevents_setup_or_doctor_on_modified_content(self):
        (self.capsule / "content/shared/skills/unslop/WORKFLOW.md").write_text("Changed")
        before = self.files()
        result = self.run_cli("setup", success=False)
    ...
    def test_hook_keeps_package_content_immutable(self):
        self.run_cli("setup")
        self.run_cli("auto", "on")
        before = {str(p.relative_to(self.capsule)): p.read_bytes() for p in self.capsule.rglob("*") if p.is_file()}
        self.run_cli("hook", "--host", "claude", payload={"hook_event_name": "UserPromptSubmit"})
        after = {str(p.relative_to(self.capsule)): p.read_bytes() for p in self.capsule.rglob("*") if p.is_file()}
    ...
    def test_offline_install_copies_exact_bundle_and_survives_source_removal(self):
        ...
        installed = self.project / ".agents/skills/poteto-mode"
    ...
    def test_offline_install_refuses_unowned_existing_capsule(self):
        ...
        self.assertIn("unowned skill", result.stderr)
        self.assertEqual(self.files(), before)
    ...
    def test_offline_upgrade_uses_new_bundle_and_preserves_opt_in(self):
        ...
        self.make_capsule(updated)
        (updated / "content/shared/skills/bstack-router/WORKFLOW.md").write_text("# Updated reviewed router\n")
        ...
        self.assertEqual((installed / "content/shared/skills/bstack-router/WORKFLOW.md").read_text(), "# Updated reviewed router\n")
        self.assertEqual(self.run_cli("doctor", capsule=installed)["bindings"], "passed")
    ...
    def test_offline_upgrade_rolls_back_capsule_when_setup_fails(self):
        ...
        self.make_capsule(updated)
        (updated / "content/shared/skills/bstack-router/WORKFLOW.md").write_text("# Pending router\n")
        self.refresh_manifest(updated)
    ...
    def test_offline_upgrade_preserves_unlisted_user_files_by_refusing(self):
        self.run_cli("install")
        self.write(".agents/skills/poteto-mode/user-note.md", "Preserve")
        before = self.files()
        result = self.run_cli("install", success=False)
