...
bstack packages Brenden Bishop's engineering workflow around Lauren "poteto" Tan's PStack. This release includes bstack's policy layer, customized unslop, selected engineering dependencies, portable setup controls, and the source and license records. Matt Pocock's selected SDLC and handoff sources are imported in the development repository for review and are not included in version @VERSION@.
