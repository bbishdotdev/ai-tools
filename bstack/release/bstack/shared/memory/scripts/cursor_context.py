...
def main():
    ...
    homes.add_argument('--home', type=Path, help='Isolated target home; ignores real application environment paths.')
    homes.add_argument('--profile-home', type=Path, help='Read-only hook: use this installed profile and its recorded native homes, independent of the invoking application environment.')
    args = parser.parse_args()
    try:
        profile = Profile.load(args.profile_home, installed=True) if args.profile_home else Profile.load(args.home)
        response = snapshot_response(profile)
    except (OSError, SyncError, ValueError):
        response = unavailable('The installed synchronized memory profile could not be verified.')
