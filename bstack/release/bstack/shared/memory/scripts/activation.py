...
def claude_configuration(profile):
    ...
    directory = profile.layout.claude
...
def cursor_configuration(profile):
    ...
    if before is None:
        ...
        if not directory.is_relative_to(base):
            raise SyncError(f'Cursor bridge directory must stay under {base}')
        after = before
...
def activation_changes(profile, router_change, cursor_config, claude_config, claude_directory, cursor_directory):
    ...
    hook_path, hook_before, hook_after = hooks_configuration(profile.home, profile.layout)
...
def backup_path(home, change, layout=None):
    label = change.destination + '-' + change.path.name.replace('.', '-')
    return (layout or Layout.load(home)).backups / f'{label}-{file_hash(change.before)}.bak'


def apply_changes(profile, changes):
    completed = []
    try:
        with sync_lock(profile.home, profile.layout):
