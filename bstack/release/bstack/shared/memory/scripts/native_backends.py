...
class Profile:
    ...
    @classmethod
    def load(cls, home=None, portable=False, installed=False):
        root = absolute_path(home or Path.home())
        layout = Layout.load(root, portable)
        ...
        isolated = home is not None and not installed
        if installed and not layout.portable:
            raise SyncError('The requested user memory profile is not installed')
        if layout.portable:
            raw, installed = read_config(layout.manifest)
            if raw is not None:
                homes = installed.get('native_homes')
                if not isinstance(homes, dict) or installed.get('home') != str(root):
                    raise SyncError('Portable memory manifest has invalid home bindings')
                resolved = []
                for name, variable in [('codex', 'CODEX_HOME'), ('claude', 'CLAUDE_CONFIG_DIR')]:
                    value = homes.get(name)
                    if not isinstance(value, str) or not value.startswith('/') or '\x00' in value:
                        raise SyncError('Portable memory manifest has invalid native home bindings')
                    path = absolute_path(value)
                    if isolated and not path.is_relative_to(root):
                        raise SyncError('--home requires installed native homes inside the disposable home')
                    if home is None and os.environ.get(variable) and absolute_path(os.environ[variable]) != path:
                        raise SyncError(f'{variable} differs from the installed memory profile; review setup before changing destinations')
                    resolved.append(path)
                codex, claude = resolved
        return cls(root, codex, claude, isolated, layout)
...
def cursor_startup_rule(home, directory=None, layout=None):
    layout = layout or Layout.load(home)
    directory = directory or layout.cursor
    helper = layout.runtime / 'scripts/sync.py'
    return (
        f'{CURSOR_START}\n'
...
def cursor_rule_ready(raw, home, directory, layout=None):
    ...
    return bool(always and text.count(CURSOR_START) == 1 and text.count(CURSOR_END) == 1 and cursor_startup_rule(home, directory, layout) in text)


def cursor_backend(profile):
    path = profile.layout.config
    ...
    try:
        ...
        if type(config.get('version')) is not int or config['version'] != 1 or not isinstance(cursor, dict):
            raise SyncError('Cursor file bridge configuration is missing or invalid; ' + profile.layout.setup_hint if profile.layout.portable else CURSOR_UNAVAILABLE)
        ...
        if not candidate.is_relative_to(profile.layout.cursor):
            raise SyncError(f'Cursor bridge directory must stay under {profile.layout.cursor}')
        root = candidate
        projection = inspect_projection(root / 'MEMORY.md', profile.home / '.cursor/rules/personal-memories.mdc')
        rule = read_file(rule_path)
        hook = inspect_hooks(profile.home, profile.layout)
        guards = ((rule_path, rule), (hook.path, hook.raw))
        if profile.layout.portable:
            guards += ((profile.layout.manifest, read_file(profile.layout.manifest)),)
        if not cursor_rule_ready(rule, profile.home, root, profile.layout):
            raise SyncError('Cursor requires its alwaysApply router for preloaded personal memories; ' + profile.layout.setup_hint)
        if projection.error:
