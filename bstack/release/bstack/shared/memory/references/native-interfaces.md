...
This report records the earlier source-linked installation and its version-specific runtime probes. The portable bundle reuses those adapters with different installation paths; see [portable setup](installation.md). Its installation tests do not refresh the native recall claims below.
...
After explicit approval, the global read-only `beforeSubmitPrompt` hook was activated from the permanent shared source. Cursor hot-loaded it without a window reload. A fresh local chat was asked for the revision of the synchronized memory snapshot supplied with its prompt; it returned the exact bridge SHA256 with zero tool calls. That revision was absent from the cached rule and was not included in the question, independently verifying model-visible hook context. The installed command completed in 43 ms, all nine links and three backends were ready, and repeat activation made no changes. Eight captured preexisting native, bridge, and configuration files retained their hashes. The hook implementation and synchronization suite passed 44 tests.
