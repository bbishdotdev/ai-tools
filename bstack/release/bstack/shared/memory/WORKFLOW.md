...
Synchronize approved personal memories with one shared command. Codex and Claude use their native memory files. Cursor uses an explicitly configured file bridge. Keep each product's native features enabled. Installing bstack alone does not activate memory or authorize storing personal notes. Once the user authorizes setup or an exact memory operation, do not ask again for that same action.

## Optional setup

For installation requests, follow [portable setup](references/installation.md). Use [scripts/portable.py](scripts/portable.py) to inspect the installation or preview setup. Apply its unchanged plan only within the user's authorized setup scope. Setup changes user-wide configuration, preserves existing notes, and never migrates their content. A detected legacy installation stays in place; portable setup does not replace it.

The active user skill is `~/.agents/skills/memory-policy`. Use its scripts for note operations, even when this document was read from a project bundle. Project updates and removal do not own that runtime. Grok has no memory adapter in this component.
...
4. Check the native interfaces before changing a destination. Read [native-interfaces.md](references/native-interfaces.md) and run `python3 ~/.agents/skills/memory-policy/scripts/portable.py status` for local installation status, then `python3 ~/.agents/skills/memory-policy/scripts/sync.py status` for adapter readiness. Recheck compatibility after product updates. File presence does not prove native recall.
...
For an explicitly approved partial save, place `--destinations codex,claude` before the command. Never add that option merely to work around a failed all-three preflight. `--home` is for disposable test profiles. Do not edit `personal-memories.mdc` by hand. It is generated from the managed bridge entries; arbitrary text outside those entries is not copied into it. The helper manages only its own marked entries and topic files; it does not migrate or remove arbitrary preexisting memories. For a portable installation, preview `scripts/portable.py setup` and apply its reviewed plan to repair an already-authorized setup. For a source-linked legacy installation, use its original `scripts/install.py --activate-sync` preview and `--apply` after checking the plan. Do not convert a legacy installation implicitly. Native generation settings must remain unchanged.
