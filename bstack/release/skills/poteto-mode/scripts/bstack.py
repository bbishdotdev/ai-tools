#!/usr/bin/env python3
"""Configure the installed bstack capsule for one local project."""
...
def package_check(capsule):
    ...
    router = manifest.get("entrypoints", {}).get("router")
    if not isinstance(router, str) or router not in seen:
        errors.append("Manifest router entrypoint must identify a hashed package file")
    if errors:
...
def build_records(project, capsule, hosts, auto, old, offline=False):
    router = capsule / "content/shared/skills/bstack-router/WORKFLOW.md"
    unslop = capsule / "content/shared/skills/unslop/WORKFLOW.md"
    script = capsule / "scripts/bstack.py"
    ...
    bodies = {
        "bstack-auto": wrapper("bstack-auto", "Explicitly enable, disable, or inspect bstack automatic routing for this project.",
            "Run the installed bstack command with the user's explicit on, off, or status argument. "
            "If no argument is supplied, use status. Never infer permission to enable automatic routing.\n\n"
            f"```sh\n{invocation} auto status {project_arg}\n```\n\n"
            "Replace `status` with the requested `on` or `off`. Report command output and any reload or native trust requirement."),
        "verify-bstack": wrapper("verify-bstack", "Check this installed bstack package and its project bindings.",
            f"Run `doctor` from the installed package.\n\n```sh\n{invocation} doctor {project_arg}\n```\n\n"
            "Then run the packaged installation verifier for an isolated offline install. "
            "Add `--methods offline symlink copy` to also check an available skills.sh distributor.\n\n"
            f"```sh\npython3 {shlex.quote(str(capsule / 'content/shared/skills/verify-bstack/scripts/installation.py'))}\n```\n\n"
            "Add `--live` only when provider-backed CLI verification is requested and model usage is authorized. "
            "That exercises manual entry, default-off behavior, automatic reminders, continuation, and opt-out. "
            "Report native trust or unavailable CLI limitations accurately."),
        "unslop": wrapper("unslop", "Apply bstack's customized unslop prose instructions.",
            f"Read `{unslop}` in full and apply it to prose. This is bstack's customized replacement; do not load an upstream unslop copy.", manual=False),
    }
    for name, text in bodies.items():
        add(f".agents/skills/{name}/SKILL.md", {"kind": "file", "text": text})
        add(f".agents/skills/{name}/agents/openai.yaml", {"kind": "file", "text": f"policy:\n  allow_implicit_invocation: {str(name == 'unslop').lower()}\n"})
    for host in hosts:
        directory = {"claude": ".claude", "cursor": ".cursor", "grok": ".grok"}.get(host)
        if directory:
            for name in (*bodies, *(("poteto-mode",) if offline else ())):
    ...
    if auto:
        ...
        instruction += ("Engineering routing is manual. Load Poteto Mode, the bstack engineering router, and its workflow "
                        "dependencies only when the user explicitly invokes Poteto Mode. Continue an explicitly invoked "
                        "workflow across turns. Otherwise follow ordinary project guidance without activating bstack "
...
def configure(project, capsule, action, hosts=None, mode=None, installation=None, locked=False):
    with nullcontext() if locked else project_lock(project):
        ...
        if action == "auto" and current["capsule"] != str(capsule):
            raise Conflict("Run auto from the configured installed capsule, or run setup explicitly to rebind it")
        old = current["managed"] if current else {}
        if action == "uninstall":
            desired = {}
            state = None
        else:
            package_check(capsule)
            ...
            desired = build_records(project, capsule, chosen_hosts, enabled, old, offline=install_kind == "offline")
...
def install_offline(project, capsule, hosts=None):
    ...
    target = safe_path(project, ".agents/skills/poteto-mode")
    with project_lock(project):
        current = state_read(project)
        if target.is_symlink():
            raise Conflict(f"Offline install will not replace a skill symlink: {target}")
        if target.exists() and target != capsule:
            if not current or current.get("installation") != "offline" or current["capsule"] != str(target):
                raise Conflict(f"Offline install will not replace an unowned skill: {target}")
            package_check(target)
        chosen_hosts = list(dict.fromkeys(hosts if hosts is not None else current["hosts"] if current else HOSTS))
        enabled = current["auto"] if current else False
        previous = current["managed"] if current else {}
        desired = build_records(project, target, chosen_hosts, enabled, previous, offline=True)
        reconcile(project, previous, desired)
        if target == capsule:
            result = configure(project, target, "setup", hosts=chosen_hosts, installation="offline", locked=True)
            result["installation"] = "offline"
            result["capsule"] = str(target)
            return result
...
def status(project, capsule, check=False):
    ...
    result = {"installed": current is not None, "auto": current["auto"] if current else False,
              ...
              "capsule": current["capsule"] if current else str(capsule), "notice": RELOAD,
              "native_hook_trust": "not_checked"}
    if check:
        package_check(capsule)
        if current:
            if current["capsule"] != str(capsule):
                raise Conflict("This command is from another capsule location; rerun setup to rebind the project")
            expected = build_records(project, capsule, current["hosts"], current["auto"], current["managed"], offline=current.get("installation") == "offline")
...
def run_hook(project, capsule, host):
    ...
    if current["capsule"] != str(capsule):
        print("{}")
        print("bstack stale hook ignored; run setup from the installed capsule", file=sys.stderr)
        return 0
    path = capsule / "runtime/reminder.py"
    ...
    module.ROOT = project
    module.ROUTER = capsule / "content/shared/skills/bstack-router/WORKFLOW.md"
...
def main():
    ...
    try:
        ...
        if args.command == "install":
            ...
            result = configure(project, capsule, "setup", hosts=args.hosts)
        elif args.command == "uninstall":
            result = configure(project, capsule, "uninstall")
